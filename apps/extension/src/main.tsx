        ...
        {pdf && <div className={`reader-subbar${quickToolbarOpen ? " open" : ""}${quickToolbarCompact ? " compact" : " complete"}`}>
          ...
          <div className="reader-subbar-content">
            <div className="reader-subbar-actions reader-subbar-left">
              <button className={`reader-tool${visualMode ? " active" : ""}`} title={visualMode ? "退出截图解析" : "截图解析：框选图片、表格或公式进行 AI 分析"} onClick={() => { setVisualMode(value => !value); setSelections([]); }}><Crop size={16}/><span>{visualMode ? "退出截图解析" : "截图解析"}</span></button>
