...
function pptErrorMessage(error: unknown) { const code = String(error || ""); if (code.includes("PPT_CREDITS_INSUFFICIENT")) return "本月 6 次免费 PPT 额度已用完，当前积分不足 20 分。"; if (code.includes("PRO_REQUIRED")) return "AI PPT 仅限有效的 Pro 会员使用。"; return code || "AI_PPT_FAILED"; }
...
export function PptStudio({ papers, extractText }: { papers: LibraryPaper[]; extractText: (paper: LibraryPaper) => Promise<string> }) {
  ...
  const [isPro, setIsPro] = useState<boolean | null>(null), [freePptRemaining, setFreePptRemaining] = useState<number | null>(null);
  ...
  useEffect(() => { void functionRequest("usage").then(async response => { const payload = await response.json().catch(() => ({})); setIsPro(response.ok && payload.plan === "pro"); setFreePptRemaining(typeof payload.aiPptFreeRemaining === "number" ? payload.aiPptFreeRemaining : null); }).catch(() => setIsPro(false)); }, []);
  ...
  if (isPro === false) return <section className="ppt-studio ppt-studio-v2 ppt-pro-gate"><header><div><span><Presentation size={15}/> AI PRESENTATION</span><h1>AI PPT 制作</h1><p>将文献转为可编辑、可下载的演示文稿。</p></div></header><div className="ppt-pro-card"><div className="ppt-pro-icon"><Crown size={28}/></div><span>PRO FEATURE</span><h2>AI PPT 制作仅对 Pro 开放</h2><p>开通 Pro 后即可从 PDF 创建演示文稿，并使用 Markdown 编辑、实时预览和下载功能。</p><small>请在右上角账户中心开通或续费 Pro 会员。</small></div></section>;
  ...
  return <section className="ppt-studio ppt-studio-v2">
    <header><div><span><Presentation size={15}/> AI PRESENTATION</span><h1>AI PPT 制作</h1><p>选择文献并创建任务；Markdown、PPT 预览和下载都在任务详情中查看。</p><small className="ppt-quota"><Crown size={13}/>Pro：{freePptRemaining === null ? "每月含 6 次免费生成" : `本月剩余 ${freePptRemaining} 次免费生成`}，超出后 20 积分 / 次</small></div></header>
