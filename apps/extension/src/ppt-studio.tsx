...
export function PptStudio({ papers, extractText }: { papers: LibraryPaper[]; extractText: (paper: LibraryPaper) => Promise<string> }) {
  ...
  useEffect(() => { void functionRequest("usage").then(async response => { const payload = await response.json().catch(() => ({})); setIsPro(response.ok && payload.plan === "pro"); }).catch(() => setIsPro(false)); }, []);
