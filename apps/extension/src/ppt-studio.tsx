...
export function PptStudio({ papers, extractText }: { papers: LibraryPaper[]; extractText: (paper: LibraryPaper) => Promise<string> }) {
  ...
  useEffect(() => { if (!result.pptId || result.fileUrl || state !== "polling") return; const check = () => { void pptRequest("status", { pptId: result.pptId }).then(async incoming => { const next = mergeResult(result, incoming); setResult(next); await updateSlides(next); saveJob(next); if (next.fileUrl || (next.total && next.current && next.current >= next.total)) setState("done"); }).catch(() => undefined); }; check(); const timer = window.setInterval(check, 2200); return () => window.clearInterval(timer); }, [result, state]);
