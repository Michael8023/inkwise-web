...
import { Download, FileText, History, LoaderCircle, MonitorPlay, Presentation, Sparkles, StopCircle } from "lucide-react";
...
type HistoryEntry = ActiveJob & { id: string; paperTitle: string; createdAt: string; fileUrl: string };
...
export function PptStudio({ papers, extractText }: { papers: LibraryPaper[]; extractText: (paper: LibraryPaper) => Promise<string> }) {
  ...
  const [result, setResult] = useState<Result>({}), [slides, setSlides] = useState<Slide[]>([]), [state, setState] = useState<State>("idle"), [error, setError] = useState(""), [completeNotice, setCompleteNotice] = useState(""), [history, setHistory] = useState<HistoryEntry[]>([]), [historyOpen, setHistoryOpen] = useState(false);
  ...
  useEffect(() => { if (!result.fileUrl || !paper) return; setCompleteNotice("PPT 已生成完成，可以下载了。 "); setHistory(current => { if (current.some(item => item.fileUrl === result.fileUrl)) return current; const next = [{ id: crypto.randomUUID(), pptId: result.pptId || "", paperId: paper.id, paperTitle: paper.title, markdown, result, slides, mode, fileUrl: result.fileUrl!, createdAt: new Date().toISOString() }, ...current].slice(0, 30); persistHistory(next); return next; }); }, [result.fileUrl]);
  ...
  return <section className="ppt-studio ppt-studio-v2">
    ...
    <section className={`ppt-history${historyOpen ? " open" : ""}`}><button className="ppt-history-head" onClick={() => setHistoryOpen(value => !value)}><span><History size={16}/>生成记录 <b>{history.length}</b></span><small>{historyOpen ? "收起" : "查看历史生成的 PPT"}</small></button>{historyOpen && <div className="ppt-history-list">{history.length ? history.map(item => <article key={item.id}><button onClick={() => { setPaperId(item.paperId); setMarkdown(item.markdown); setResult(item.result); setSlides(item.slides || []); setMode(item.mode); setState("done"); setCompleteNotice(""); }}><Presentation size={17}/><span><b>{item.paperTitle}</b><small>{new Date(item.createdAt).toLocaleString("zh-CN")} · {item.mode === "markdown" ? "Markdown 转 PPT" : item.mode === "stream" ? "流式生成" : "后台生成"}</small></span></button><a href={item.fileUrl} target="_blank" rel="noreferrer" title={item.fileUrl}><Download size={15}/>File URL</a></article>) : <p>还没有完成的 PPT 记录。</p>}</div>}</section>
