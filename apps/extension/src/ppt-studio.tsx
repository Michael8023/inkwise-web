...
import { ArrowLeft, Crown, Download, FileText, History, MonitorPlay, Presentation, Sparkles } from "lucide-react";
...
function pptErrorMessage(error: unknown) { const code = String(error || ""); if (code.includes("PPT_CREDITS_INSUFFICIENT")) return "本月免费 PPT 额度已用完，当前积分不足 20 分。"; if (code.includes("PPT_TRIAL_USED")) return "免费试用已经使用过，开通 Pro 后可继续制作 PPT。"; if (code.includes("PRO_REQUIRED")) return "AI PPT 仅限有效的 Pro 会员使用。"; return code || "AI_PPT_FAILED"; }
async function pptRequest(action: string, request: Record<string, unknown>) { const response = await functionRequest("ai-ppt", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, request }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(pptErrorMessage(payload.error)); return payload as Result; }
async function pptStream(action: "outline" | "content", request: Record<string, unknown>, onText: (value: string) => void, onPptId: (value: string) => void) {
  ...
  if (!response.ok || !response.body) { const payload = await response.json().catch(() => ({})); throw new Error(pptErrorMessage(payload.error || "AI_PPT_STREAM_FAILED")); }
...
function persistHistory(entries: HistoryEntry[], storageKey: string) { try { localStorage.setItem(storageKey, JSON.stringify(entries.slice(0, 30))); } catch { /* Storage is optional. */ } }

export function PptStudio({ papers, extractText, userId }: { papers: LibraryPaper[]; extractText: (paper: LibraryPaper) => Promise<string>; userId: string }) {
  const historyStorageKey = `shidea-ai-ppt-history:${userId}`;
  ...
  const [isPro, setIsPro] = useState<boolean | null>(null), [trialUsed, setTrialUsed] = useState<boolean | null>(null), [freePptRemaining, setFreePptRemaining] = useState<number | null>(null);
  const historyRef = useRef<HistoryEntry[]>([]), watchingRef = useRef(new Set<string>()), generatingRef = useRef(false);
  ...
  useEffect(() => { void functionRequest("usage").then(async response => { const payload = await response.json().catch(() => ({})); setIsPro(response.ok && payload.plan === "pro"); setTrialUsed(Boolean(payload.aiPptTrialUsed)); setFreePptRemaining(typeof payload.aiPptFreeRemaining === "number" ? payload.aiPptFreeRemaining : null); }).catch(() => { setIsPro(false); setTrialUsed(true); }); }, []);
  useEffect(() => {
    // Discard the legacy shared key once. It was not scoped to an account.
    localStorage.removeItem("shidea-ai-ppt-history");
    try { const savedHistory = JSON.parse(localStorage.getItem(historyStorageKey) || "[]"); if (Array.isArray(savedHistory)) setHistory(savedHistory.map(item => ({ ...item, status: item.status || (item.fileUrl ? "completed" : item.mode === "markdown" ? "editing" : "generating") }))); } catch { persistHistory([], historyStorageKey); }
  }, [historyStorageKey]);
  useEffect(() => { historyRef.current = history; }, [history]);
  const updateEntry = (id: string, patch: Partial<HistoryEntry>) => setHistory(current => { const next = current.map(item => item.id === id ? { ...item, ...patch } : item); historyRef.current = next; persistHistory(next, historyStorageKey); return next; });
  const createEntry = (status: JobStatus) => { const id = crypto.randomUUID(); const entry: HistoryEntry = { id, pptId: "", paperId, paperTitle: paper?.title || "未命名文献", markdown: "", result: {}, slides: [], mode, createdAt: new Date().toISOString(), status }; setHistory(current => { const next = [entry, ...current].slice(0, 30); historyRef.current = next; persistHistory(next, historyStorageKey); return next; }); return id; };
  ...
  async function generateMarkdown(alsoGeneratePpt: boolean) { if (generatingRef.current || ["reading", "generating", "polling"].includes(state)) return; generatingRef.current = true; const entryId = createEntry("generating"); setError(""); if (alsoGeneratePpt && isPro && freePptRemaining === 0) setCompleteNotice("本月免费 PPT 额度已用完，本次生成将扣除 20 积分。"); setMarkdown(""); setResult({}); setSlides([]); setState("reading"); try { const subject = await instruction(); setState("generating"); const setText = (text: string) => { setMarkdown(text); updateEntry(entryId, { markdown: text }); }; const outline = await pptStream("outline", { subject }, setText, () => undefined); let taskId = ""; const setTask = (id: string) => { taskId = id; const taskResult = { pptId: id }; updateEntry(entryId, { pptId: id, result: taskResult, status: "generating" }); void watchEntry(entryId, id); }; const content = await pptStream("content", { outlineMarkdown: outline, asyncGenPptx: alsoGeneratePpt, ...(alsoGeneratePpt ? { billingRequestId: crypto.randomUUID() } : {}) }, setText, setTask); const finalMarkdown = content || outline; setMarkdown(finalMarkdown); if (alsoGeneratePpt && !taskId) throw new Error("PPT 任务未返回任务编号，请重试。"); updateEntry(entryId, { markdown: finalMarkdown, status: alsoGeneratePpt ? "generating" : "editing" }); setState(alsoGeneratePpt ? "polling" : "editing"); } catch (caught) { const message = caught instanceof Error ? caught.message : "Markdown 生成失败，请稍后重试。"; setState("error"); setError(message); updateEntry(entryId, { status: "failed", error: message }); } finally { generatingRef.current = false; } }
  async function directGenerate() { await generateMarkdown(true); }
  async function generatePptForEntry(entry: HistoryEntry) { setError(""); if (isPro && freePptRemaining === 0) setCompleteNotice("本月免费 PPT 额度已用完，本次生成将扣除 20 积分。"); updateEntry(entry.id, { status: "generating", error: undefined }); try { let taskId = ""; await pptStream("content", { outlineMarkdown: entry.markdown, asyncGenPptx: true, billingRequestId: crypto.randomUUID() }, () => undefined, id => { taskId = id; const taskResult = { ...entry.result, pptId: id }; updateEntry(entry.id, { pptId: id, result: taskResult, status: "generating" }); void watchEntry(entry.id, id); }); if (!taskId) throw new Error("PPT 任务未返回任务编号，请重试。"); setState("polling"); } catch (caught) { const message = caught instanceof Error ? caught.message : "PPT 生成失败，请稍后重试。"; updateEntry(entry.id, { status: "failed", error: message }); } }
  ...
  if (isPro === false && trialUsed === true) return <section className="ppt-studio ppt-studio-v2 ppt-pro-gate"><header><div><span><Presentation size={15}/> AI PRESENTATION</span><h1>AI PPT 制作</h1><p>将文献转为可编辑、可下载的演示文稿。</p></div></header><div className="ppt-pro-card"><div className="ppt-pro-icon"><Crown size={28}/></div><span>PRO FEATURE</span><h2>免费试用已使用</h2><p>开通 Pro 后即可继续从 PDF 创建演示文稿，并使用 Markdown 编辑、实时预览和下载功能。</p><small>请在右上角账户中心开通或续费 Pro 会员。</small></div></section>;
