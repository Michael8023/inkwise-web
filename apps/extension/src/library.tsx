...
import {
  ...
  Heart, LayoutGrid, List, MoreHorizontal, Pencil, Search, Star, Trash2, Upload, X, ChevronLeft, Presentation, UserRound,
} from "lucide-react";
...
export function LibraryScreen({ onClose, onOpen, onImportFile, onImportUrl, extractText, onOpenAccount, userId, canReturn = true }: {
  onClose: () => void; onOpen: (paper: LibraryPaper) => void; onImportFile: (file: File) => void;
  onImportUrl: (url: string) => Promise<void>; extractText: (paper: LibraryPaper) => Promise<string>; onOpenAccount: () => void; userId: string; canReturn?: boolean;
}) {
  ...
  const currentUserId = async () => { const { data } = await supabase.auth.getUser(); if (!data.user) throw new Error("AUTH_REQUIRED"); return data.user.id; };
  const submitDialog = async () => {
    ...
    if (dialog?.kind === "folder") {
      ...
      else { try { const result = await supabase.from("library_folders").insert({ name: value.slice(0, 120), parent_id: dialog.parentId || null, user_id: await currentUserId() }); if (result.error) throw result.error; void reload(); } catch { setError("文件夹创建失败，该位置可能已有同名文件夹。"); } }
    }
    if (dialog?.kind === "tag") { const result = dialog.tag ? await supabase.from("library_tags").update({ name: value.slice(0, 48) }).eq("id", dialog.tag.id) : await supabase.from("library_tags").insert({ name: value.slice(0, 48), color: "#0e9f9a", user_id: await currentUserId() }); if (result.error) setError(dialog.tag ? "标签保存失败。" : "标签创建失败，该名称可能已存在。"); else void reload(); }
    if ((dialog?.kind === "delete" || dialog?.kind === "deleteMany") && (dialog.paper || dialog.papers)) {
      const targets = dialog.papers || [dialog.paper!];
      const result = await supabase.from("library_papers").delete().in("id", targets.map(item => item.id));
      if (result.error) setError("文献记录删除失败，请稍后重试。");
      else {
        const storage = await supabase.storage.from("library-pdfs").remove(targets.map(item => item.storage_path));
        if (storage.error) setError("文献记录已删除，但 PDF 清理失败。请联系支持处理。");
        setSelected([]); void reload();
      }
    }
