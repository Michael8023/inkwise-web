...
import {
  Archive, Check, ChevronDown, FilePlus2, FileText, Folder, FolderPlus, FolderSearch,
  Heart, LayoutGrid, List, MoreHorizontal, Pencil, Search, Star, Trash2, Upload, X,
} from "lucide-react";
...
type View = "all" | "favorite" | "recent" | "archived" | string;
type Sort = "recent" | "added" | "title" | "size";
type Dialog = null | { kind: "rename" | "folder" | "delete" | "deleteMany" | "folderDelete"; paper?: LibraryPaper; papers?: LibraryPaper[]; folder?: LibraryFolder };
...
export function LibraryScreen({ onClose, onOpen, onImportFile, onImportUrl, canReturn = true }: {
  onClose: () => void; onOpen: (paper: LibraryPaper) => void; onImportFile: (file: File) => void;
  onImportUrl: (url: string) => Promise<void>; canReturn?: boolean;
}) {
  ...
  const visible = useMemo(() => papers.filter(paper => {
    ...
    if (view === "recent" && !paper.library_paper_states?.reader_state?.currentPage) return false;
    if (!(["all", "favorite", "recent", "archived"] as string[]).includes(view) && paper.folder_id !== view) return false;
  ...
  const createTag = async () => { const value = tagDraft.trim(); if (!value) return; try { const result = await supabase.from("library_tags").insert({ name: value, color: "#0e9f9a", user_id: await userId() }); if (result.error) throw result.error; setTagDraft(""); void reload(); } catch { setError("标签创建失败，该名称可能已存在。"); } };
  const submitDialog = async () => {
    const value = name.trim(); if ((dialog?.kind === "rename" || dialog?.kind === "folder") && !value) return;
    ...
    if (dialog?.kind === "folder") {
      ...
      else { try { const result = await supabase.from("library_folders").insert({ name: value.slice(0, 120), parent_id: null, user_id: await userId() }); if (result.error) throw result.error; void reload(); } catch { setError("文件夹创建失败，该名称可能已存在。"); } }
    }
  ...
  const Paper = ({ paper }: { paper: LibraryPaper }) => <article className={`library-paper ${layout === "grid" ? "library-paper-grid" : ""}`} onClick={() => onOpen(paper)}>
    ...
    <div className="library-paper-actions" onClick={event => event.stopPropagation()}><button aria-label="更多操作" onClick={() => setMenu(menu === paper.id ? null : paper.id)}><MoreHorizontal size={18}/></button>{menu === paper.id && <div className="library-menu"><button onClick={() => void update([paper.id], { is_favorite: !paper.is_favorite })}><Star size={15}/>{paper.is_favorite ? "取消收藏" : "收藏"}</button><button onClick={() => { setName(paper.title); setDialog({ kind: "rename", paper }); setMenu(null); }}><Pencil size={15}/>编辑标题</button><label><Folder size={15}/>移动到<select value={paper.folder_id || ""} onChange={event => void update([paper.id], { folder_id: event.target.value || null })}><option value="">未分类</option>{folders.map(folder => <option value={folder.id} key={folder.id}>{folder.name}</option>)}</select></label><div className="library-menu-tags"><span>标签</span>{tags.map(tag => <button key={tag.id} className={paper.library_paper_tags?.some(item => item.library_tags?.id === tag.id) ? "active" : ""} onClick={() => void toggleTag(paper, tag)}>{tag.name}</button>)}</div><button onClick={() => void update([paper.id], { archived_at: paper.archived_at ? null : new Date().toISOString() })}><Archive size={15}/>{paper.archived_at ? "恢复文献" : "归档"}</button><button className="danger" onClick={() => { setDialog({ kind: "delete", paper }); setMenu(null); }}><Trash2 size={15}/>永久删除</button></div>}</div>
  </article>;
