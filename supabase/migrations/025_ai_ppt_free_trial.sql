-- Free accounts can generate one PPT as a lifetime trial. Subsequent PPT work
-- requires Pro; Pro accounts retain the quota defined in migration 024.
...
alter table public.usage_ledger add constraint usage_ledger_feature_check
  check (feature in ('summary_short','summary_full','explain','chat','figure_explain','table_extract','ai_ppt','ai_ppt_trial'));
...
begin
  ...
  if entitlement.user_id is null or entitlement.status <> 'active' then
    return jsonb_build_object('ok', false, 'error', 'PRO_REQUIRED');
  end if;

  if plan_name <> 'pro' or entitlement.period_end <= now() then
    select count(*) into trial_count from public.usage_ledger
      where user_id = p_user_id and feature = 'ai_ppt_trial' and status <> 'failed';
    if trial_count >= 1 then return jsonb_build_object('ok', false, 'error', 'PPT_TRIAL_USED'); end if;
    insert into public.usage_ledger(user_id, feature, model, credits, request_id, input_chars, status)
    values (p_user_id, 'ai_ppt_trial', 'docmee-ppt-generate', 0, p_request_id, greatest(p_input_chars, 0), 'reserved');
    return jsonb_build_object('ok', true, 'trial', true, 'chargedCredits', 0, 'creditsRemaining', entitlement.credits_remaining);
  end if;

  select count(*) into used_count
  from public.usage_ledger
  where user_id = p_user_id and feature = 'ai_ppt' and status <> 'failed'
  ...
  if used_count >= 6 then
    ...
    update public.user_entitlements set credits_remaining = credits_remaining - charge, updated_at = now()
      where user_id = p_user_id returning * into entitlement;
  end if;

  insert into public.usage_ledger(user_id, feature, model, credits, request_id, input_chars, status)
  values (p_user_id, 'ai_ppt', 'docmee-ppt-generate', charge, p_request_id, greatest(p_input_chars, 0), 'reserved');
  return jsonb_build_object('ok', true, 'freeUsed', used_count + 1,
